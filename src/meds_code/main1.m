...
max_iter = n_samples; % Maximum number of iterations for updates
convergence_threshold = 1e-4; % Convergence threshold for stopping
...
%% Step 2: Sequential updating (Martingale Posterior) with Convergence Check
for n = 1:n_samples
    ...
    % Update beta
    beta_new = beta + eps_n * (xn * (yn - xn' * beta));
    
    % Update sigma^2
    sigma2_new = sigma2 + eps_n * ((yn - xn' * beta)^2 - sigma2);
    
    % Update mu_X
    mu_X_new = mu_X + eps_n * (xn - mu_X);
    
    % Update Sigma_X
    Sigma_X_new = Sigma_X + eps_n * ((xn - mu_X) * (xn - mu_X)' - Sigma_X);
    
    % Enforce symmetry in Sigma_X to prevent numerical drift
    Sigma_X_new = 0.5 * (Sigma_X_new + Sigma_X_new');
    
    % Store history for visualization
    beta_history(:, n) = beta_new;
    sigma2_history(n) = sigma2_new;
    mu_X_history(:, n) = mu_X_new;
    Sigma_X_history(:, :, n) = Sigma_X_new;

    % Check for convergence (using change in parameter estimates)
    if n > 1 && ...
       max(abs(beta_new - beta)) < convergence_threshold && ...
       abs(sigma2_new - sigma2) < convergence_threshold && ...
       max(abs(mu_X_new - mu_X)) < convergence_threshold && ...
       max(abs(Sigma_X_new(:) - Sigma_X(:))) < convergence_threshold
